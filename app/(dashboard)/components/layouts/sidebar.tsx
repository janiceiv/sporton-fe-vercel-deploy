    ...
    <div className="flex flex-col gap-2 mt-12 p-5">
      {
        menuItems.map((item,index) => {
          ...
          return (
            <Link key={index} href={item.link} className={`flex gap-3 items-center py-3 px-4.5 font-medium ${isActive ? 'bg-primary/15 text-primary rounded-lg':'hover:bg-gray-100'}`}>
              <item.icon size={24}/>
              <span>{item.name}</span>
            </Link>           
          )
