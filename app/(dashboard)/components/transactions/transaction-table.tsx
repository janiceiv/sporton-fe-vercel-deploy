...
const TransactionTable = ({ onViewDetails , transactions}: TTransactionModalProps) => {
        ...
        <tbody>
        {
          transactions.map((data)=>(
            <tr key={data._id} className="border-b border-gray-200 last:border-b-0">
              ...
              <td className="px-6 py-4 font-medium">
                <div className={`px-4 py-1 rounded-full border border text-center w-fit text-sm uppercase ${getStatusColor(data.status)}`}>{data.status} </div>
              </td>
